    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    {
        // 判断一个3D点远/近的阈值 mbf * 35 / fx; Baseline times.
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp)
{
    ...
    *   1. Current Frame ID
        ...
        4. Undistort特征点
               UndistortKeyPoints()
        5. 计算双目间的匹配, 匹配成功的特征点会计算其深度
           ComputeStereoMatches()
        6. first Frame
          ComputeImageBounds()
          camera intrisics
        7. AssignFeaturesToGrid()
    *
    */
...
//    vector<ORB_SLAM2::KeyFrame*> cand = mpKeyFrameDB->DetectRelocalizationCandidates(&mCurrentFrame);
...
void Tracking::Track()
{
    ...
    if(mState==NOT_INITIALIZED)
    {
        ...
        if(mSensor==System::STEREO || mSensor==System::RGBD)
            StereoInitialization();     // stereo & RGB-D => mState=OK;
        else
            MonocularInitialization();
//        // Load Map
//        if (mpViewer->mInitLocalizationMode)
        ...
        else
        {
            ...
            else
            {
                ...
                if(!mbVO)
                {
                    ...
                    // mbVO为false则表明此帧匹配了很多的3D map点，非常好
                    if(!mVelocity.empty())
                    {
                        bOK = TrackWithMotionModel();
                        // 这个地方是不是应该加上：
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        // 步骤4：在局部地图中添加该初始关键帧
        mpLocalMapper->InsertKeyFrame(pKFini);  // mlNewKeyFrames.push_back(pKF);
        ...
        mpReferenceKF = pKFini; //king： 初始帧作为关键帧，作为参考关键帧。
 ...
 * @brief 单目的地图初始化
 *
 * 并行地计算基础矩阵和单应性矩阵，选取其中一个模型，恢复出最开始两帧之间的相对姿态(R,t)以及点云Pw
 * 得到初始两帧的匹配、相对运动、初始MapPoints
 */
void Tracking::MonocularInitialization()
{
    // 如果单目初始器还没有被创建，则创建单目初始器 // first frame
    if(!mpInitializer)
    {
        // Set Reference Frame
        // 单目初始帧的特征点数必须大于100
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // king: std::vector<int> mvIniMatches;// 跟踪初始化时前两帧之间的匹配
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        //Create MapPoint.
        cv::Mat worldPos(mvIniP3D[i]);  // std::vector<cv::Point3f> mvIniP3D;	// 存放3D点的临时变量
        ...
        // 步骤4.3：表示该KeyFrame的哪个特征点可以观测到the3D点
        ...
        // c.更新该MapPoint平均观测方向以及观测距离的范围
        // king: mNormalVector; mfMinDistance, mfMaxDistance
        ...
        //Add to Map
        // 步骤4.4：在地图中添加该MapPoint
        mpMap->AddMapPoint(pMP);    // king: mspMapPoints.insert(pMP);
    }

    // Update Connections
    // 步骤5：更新关键帧间的连接关系
    // 在3D点和关键帧之间建立边，每个边有一个权重，边的权重是该关键帧与当前帧公共3D点的个数
    // 在没有执行这个函数前，关键帧只和MapPoints之间有连接关系，这个函数可以更新关键帧KFs之间的连接关系
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    // 计算mBowVec和mFeatVec，其中mFeatVec记录了属于第i个node（在第4层）的ni个描述子
    mCurrentFrame.ComputeBoW();
...
{
    // king: 0.9 ratio of the best and the second score
    ORBmatcher matcher(0.9,true);
    ...
    mCurrentFrame.SetPose(mVelocity*mLastFrame.mTcw);   // cv::Mat mVelocity;
...
void Tracking::SearchLocalPoints()
{
    ...
    for(vector<MapPoint*>::iterator vit=mvpLocalMapPoints.begin(), vend=mvpLocalMapPoints.end(); vit!=vend; vit++)
    {
        ...
        if(mCurrentFrame.isInFrustum(pMP,0.5))  // cos60 = 0.5
        {
...
bool Tracking::Relocalization()
{
    ...
    // If enough matches are found we setup a PnP solver
    // king: ratio of the best and the second score
    ...
    vvpMapPointMatches.resize(nKFs);    // king: vvpMapPointMatches[nKFs][]
